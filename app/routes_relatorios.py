    ...
    def _processar_resultados_agregados(self, resultados):
        ...
        for row in resultados:
            ...
            if cat_id not in categorias:
                categorias[cat_id] = {'id': f'cat-{cat_id}', 'codigo': cat_id, 'descricao': row['nome_categoria'], 'nivel': 0, 'classes': 'nivel-0', 'fontes': {}, **self._valores_base()}
        ...
        def calc_variacao(item):
            item['variacao_absoluta'] = item['receita_atual'] - item['receita_anterior']
            item['variacao_percentual'] = (item['variacao_absoluta'] / item['receita_anterior'] * 100) if item.get('receita_anterior') != 0 else 0
            return item
